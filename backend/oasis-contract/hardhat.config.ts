...
task('deploy')
  .setAction(async (args, hre) => {
    ...
    const ReviewToken = await hre.ethers.getContractFactory('ReviewToken', new hre.ethers.Wallet(accounts[0], uwProvider));
    const reviewToken = await ReviewToken.deploy();
    await reviewToken.waitForDeployment();

    console.log(`ReviewToken address: ${await reviewToken.getAddress()}`);
    return reviewToken;
});

// Read message from the ReviewToken.
task('message')
  .addPositionalParam('address', 'contract address')
  .setAction(async (args, hre) => {
    await hre.run('compile');

    const reviewToken = await hre.ethers.getContractAt('ReviewToken', args.address);
    const message = await reviewToken.message();
    const author = await reviewToken.author();
...
task('setMessage')
  ...
  .setAction(async (args, hre) => {
    ...
    let reviewToken = await hre.ethers.getContractAt('ReviewToken', args.address);
    const tx = await reviewToken.setMessage(args.message);
